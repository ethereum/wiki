...
Satoshi Nakamoto's Bitcoin was a marvelous invention. While much of cryptographic research since the 1980s has been delving into greater and greater depths of complexity and obscurity, developing protocols based on arcane constructions such as ideal lattices, elliptic curve pairing functions and self-referential homomorphic encryption strategies, Bitcoin managed to solve a thirty-year-old problem in computer science using nothing but the elementary building blocks of hashes and public keys. However, although the discovery of Bitcoin was an amazing feat, the protocol was only a first draft, and since the currency and blockchain were initiated and released into the wild we have learned a number of important insights about what the practical problems with cryptocurrency, and the somewhat wider and more generic field of "digital consensus", are. There are fundamental roadblocks in scalability, economic issues around consensus stability and distribution, as well as higher-level application issues around topics like reputation systems that remain to be solved, and many of these will require yet another fundamental, zero-to-one, breakthrough in order to see much improvement. The purpose of this document is to identify some major points of pressure for decentralized consensus architectures as they stand today, and coalesce them into a set of "problems" with something reasonably close to a formal definition.

### Introduction to Cryptoeconomics

The advent of cryptocurrency brings a new, and interesting, dimension to the science of cryptography. While before cryptography was, by and large, a purely computational and information-theoretic science, with strong guarantees built on security assumptions that are close to absolute, once digital money enters the picture the perfect world of math and numbers must interact with a much more messy reality of human social structures, economic incentives and partial guarantees and known vulnerabilities that can only be mitigated, and not outright removed. While a cryptographer is used to assumptions of the form "this algorithm is guaranteed to be computationall infeasible to crack provided that all factoring algorithms and all algorithms for taking nth roots modulo composites require an amount of computation that is superpolynomial in the size of the inputs", in the world of cryptocurrency there are classes of attacks such as "51% attacks" and market manipulation attacks where the difficulty of the security assumption in question cannot be cleanly defined, and there may even be multiple equilibria depending on sociocultural circumstances. Empirical factors like the difficulty of collusion attacks, the relative efficiency of specialized hardware and the superlinearity or sublinearity of the output-cost curve for obtaining computing power are things that require heavy real-world observation to understand, and the fundamental weaknesses that occur around these issues are only partially resolvable.
...
* If N is the number of transactions, a transaction made should reach the point where a double spend on that transaction has a probability of success less than `p` in `O(log(N)*log(1/p))` time for all `p > 0`; as time approaches infinity, `p -> 0`
* The network should be able to work if no node in the network has more than `k(N)` bandwidth, computational resources and storage space, where `k(N)` is a function that is sublinear in `N`. The more highly sublinear, the better.
...
* You are allowed to rely on an existing concept of "N nodes"

**Requirements**

* The system should continuously provide a time which is within 120s (or less if possible) of the internal clock of all honestly participating nodes.
...
However, there is a problem. Although many deny it, Bitcoin's proof-of-work system is, put bluntly, in a state of crisis. Nearly 50% of all new hashpower being added to the Bitcoin network is being produced in a few factories in Shenzhen, two mining pools control [half of the network](http://blockchain.info/pools), and the system consumes tens of millions of dollars of resources annually. Security researcher Dan Kaminsky has gone on record saying, in May 2013, that he did not believe that Bitcoin's proof-of-work algorithm would not survive the next year. Although he proved to be wrong in reality, in spirit his assessement is arguably exactly correct. The question is, can we do better? Of course, there is the possibility that the problem will solve itself over time, and as the Bitcoin mining industry grows it will naturally become more decentralized as room emerges for more firms to participate. However, that is an empirical claim that may or may not come to pass, and we need to be prepared for the eventuality that it does not. Furthermore, the wasted energy and computation costs of proof of work as they stand today may prove to be entirely avoidable, and it is worth looking to see if that aspect of consensus algorithms can be alleviated.
...
* `PoWProduce` must not be superlinear in computational power or time; that is to say, the expected number of successful `PoWProduce` computations for a node with `N` dollars worth of hardware after `t` seconds should be bounded by `kNt` for some `k`. Furthermore, at a difficulty level which assumes $1 billion worth of hardware in the network producing one successful `PoWProduce` computation per minute, $1000 worth of hardware should be able to reach 50% of this maximum efficiency.
* It should be shown with reasonably rigorous technological and economic analysis that ASICs, defined as devices tailored toward the specific mining task that are not also proportionately more powerful at general computational tasks, are not expected to be more than 10x more efficient than generic CPU/GPU hardware per dollar spent on electricity and production with initial and fixed costs infinitely amortized at a 5% discount rate and assuming a firm size of $100 billion.
...
The Slasher algorithm, described [here](http://blog.ethereum.org/2014/01/15/slasher-a-punitive-proof-of-stake-algorithm/) and implemented by Zack Hess as a proof-of-concept [here](https://github.com/zack-bitcoin/slasher), represents my own attempt at fixing the second problem. The crux of the algorithm is that (1) the miners for a specific block are determined ahead of time, so in the event of a fork a miner will either have an opportunity to mine a given block on all chains or no chains, and (2) if a miner is caught signing two distinct blocks with the same block number they can be punished and forced to surrender their block reward in the 1000 blocks before it becomes available to spend. The algorithm is viable and effective, but it suffers from two flaws of unknown significance. First, if all of the miners for a given block learn each other's identities beforehand, they can meet up and collude to either perform a transaction reversal (or "double-spend") attack or extort the community for money with the threat of not signing the block and forcing the network to grind to a halt. Second, the nothing-at-stake problem remains for long-range attacks going back more than 3000 blocks.
...
**7. Human-dominated egalitarian PoW**
...
* Given a well-justified extrapolation of the global levels of human and computer competence at the underlying problem, over 75% of the voting shares in the system should be provided by human labor, either unaided or aided only by commodity hardware and free open-source software.
* Ideally, the top human laborer should not be more than 100x as effective as the bottom 10th percentile human laborer.

**8. Proof of excellence**

One interesting, and largely unexplored, solution to the wasted effort problem, and simultaneously the specialized hardware problem, is creating a proof of work that is dominated by highly intellectual human labor involving solving problems that are useful to sicety. For example, one can come up with a "proof of proof" currency that rewards players for coming up with mathematical proofs of certain theorems. There is no generic algorithm, aside from brute force, for proving theorems, and yet proofs of theorems are theoretically computationally easy to verify: one simply needs to write every step of the proof in a formal language, allowing the use of only one inference rule (eg. `a + b = b + a` or `a * (b + c) = a * b + a * c` but not `a * (b + c) = a * c + b * a`) between each step, and having a program verify the correctness of the inferences at each step.
...
The second part of cryptoeconomics, and the part that most technologists and cryptocurrency enthusiasts tend to have less experience with, is economics. Price stability is a major and unresolved problem in cryptocurrency, and one preventing many risk-averse businesses and people from dealing with Bitcoin-like platforms. Theoretically, it may be possible to create an asset which is simultaneoulsy cryptographically trust-free in the same way that Bitcoin is and has a much more constant price. Going beyond currency, there are also other base-level components that may prove to be very important in building higher-level cryptoeconomic platforms: reputation systems and anti-Sybil systems. Reputation is in many ways a counterpart to currency: currency is an absolute score, whereas reputation is person-dependent (eg. I may have a high reputation among the cryptocurrency community in North America, am virtually unknown in Guatemala, and may even be an enemy in certain anti-technologist or ultranationalist communities). The purpose of anti-Sybil mechanisms is to make it more difficult for attackers to create millions of accounts to spam or overwhelm systems; in general, this entails doing something to prove that you are a human. These problems are already hard in the real world; however, the transparency and high information access of a blockchain may actually make them easier to solve cryptoeconomically. However, doing so is hard. Another hard part about economics is that economics deals heavily with empirical issues and matters of incentive, so it is hard to come up with precise specifications for when a problem is solved; instead, all we can do is require either an economic model or a real-world simulation of a protocol actually working.
...
* All transactions in progress and finalized are recorded on a blockchain or other public data store
...
2. **Social proof** - best seen with invite-only social networks, especially those where the support of `N` individuals is required in order to enter. 

...
There are two primary reasons why DAOs may theoretically be useful. The first is that DAOs reduce the need for trust. When a product or service (including a public good) is produced by a large centralized entity, that creates a large nexus of trust with the potential to abuse it. Potential real-life examples of this include the privacy issues surrounding social networks and traditional cloud computing companies, the prevalence of principal-agent problems and systemic risks in the financial industry, and similar issues in healthcare. Additionally, even in industries where the rate of fraud is not high in practice, some may make the argument that this is the case only because of stringent regulation, and this regulation inevitably imposes high compliance costs - issues that could be entirely bypassed if protocols for those particular industries could be devised where no single individual is assumed to be trusted.

The second is that traditional organizations tend to impose a binary member/non-member distinction, with a small class of individuals spending most or all of their time working inside the organization and everyone else having at most limited involvement as a volunteer. This is arguably an unnatural and highly suboptimal way of organizing, as it fails to capture the full power-law distribution of people's desire to contribute. Particularly, it fails to contribute the full power-law distribution of people's desire, and technical ability to contribute to the organization's leadership. However, organizational structures that abolish this distinction run into a problem: they need to be highly inclusive, often sacrificing the ability to effectively filter contributors. Secure cryptoeconomic interaction protocols used in the context of decentralized organizations may help resolve these issues and make inclusive organizational paradigms more viable.
...
**18. Liquid Democracy Implementation**

Aside from futarchy, the other leading paradigm in decentralized organizational governance is the concept of "liquid democracy" (or "delegative democracy"). The idea behind liquid democracy is simple: all decisions are made via direct democracy by default, so every member of the organization, perhaps in proportion to some metric of their stake or contributions, has the ability to participate in the vote, but individuals have the ability to delegate their vote to others, perhaps including delegation by category (eg. "I assign my vote to Bob on all issues, except those issues that a 2-of-3 voting pool of Fred, George and Smith deems environmental issues; in that case, I assign my vote to Sam"). Delegation is transitive; if I assign to Bob and Bob assigns to Matt, then my votes are ultimately decided by Matt. Delegation can also be added or canceled at any time.

Note that liquid democracy can be combined with other concepts in this document; for example, per-person democracy depends on an identity system, contribution-based democracy depends on contribution metrics, and per-token democracy depends on a secure token system. Additionally, liquid democracy and futarchy can be combined - liquid democracy selects the precise success metric to use, and futarchy optimizes for it. However, there are challenges. Online voting is known to be a controversial topic, and particularly in an open cryptographically authenticated environment concerns about vote selling, bribery and coercion are heightened. It will be crucial to determine what the risks are depending on the specific use case, and how best to mitigate them.

**Problem**: develop a mechanism for governing a decentralized organization via a liquid democracy-like protocol.

**Additional Assumptions and Requirements**

* Most issues affect each participant slightly
* Some portion of participants are altruistic, but most are neutral
* The system exists in the context of a blockchain-based platform. Coercion-proof voting protocols should be designed with this in mind.

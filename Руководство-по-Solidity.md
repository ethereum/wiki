_исходный текст [Solidity Tutorial](https://github.com/ethereum/wiki/wiki/Solidity-Tutorial), 05.10.2015_
***
Solidity является высокоуровневым языком, синтаксис которого схож c JavaScript, разработанным для компиляции кода для Виртуальной машины Ethereum. Это учебное руководство обеспечивает основное введение в Solidity и предполагает некоторое знание Виртуальной машины Ethereum и программирования в целом. Оно не касается таких функций как [естественная спецификация языка](https://github.com/ethereum/wiki/wiki/Solidity-Tutorial/Ethereum-Natural-Specification-Format) или формальная верификация и не является заключительной спецификацией языка.
...
Solidity является статически типизированным языком и это означает, что тип каждой переменной (состояние и локальных) должен быть указан (или по крайней мере известен - смотри [Дедукция типа](#Дедукция-типа) ниже) во время компиляции. Solidity предоставляет несколько элементарных типов, которые могут быть объединены в составные типы.
...
`bool`: Логический тип, константы `true`, `false`, операторы: `!` (логическое отрицание) `&&` (логическая конъюнкция, "and"), `||` (логическая дизъюнкция, "or"), `==` (равенство) and `!=` (неравенство).

Операторы `||` и `&&` применяют обычные правила short-circuiting.  Это означает, что в выражении `f(x) || g(y)`, если `f(x)` оценивается как `true`, `g(y)` не будет рассмотрен даже если это может иметь побочные эффекты.
...
`address`: Содержит 20 байтное значение (размер адреса Ethereum). Операторы: `<=`, `<`, `==`, `!=`, `>=` и `>`. адресный тип также имеет члены(см. [Функции на адреса](#Функции-на-адреса)) и служит основой для всех контрактов.
...
Строковые Литералы: Строковые литералы должны быть записаны с двойными кавычками ("abc"). Как с целочисленными литералами, может варьироваться их тип, но они неявно конвертируемы к bytes • если они соответствуют bytes и к string

__Строковые Литералы__: Строковые литералы записываются с двойными кавычками (`"abc"`). ак с целочисленными литералами, тип может варьироваться,но они неявно конвертируемы в `bytes•` если помещаются, в `bytes` и в `string`.

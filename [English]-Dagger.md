...
# Let `H[0] = D+N` where D is the underlying data and N is the nonce.
# For `i` from `1` to `N` let `H[i] = f(H[0] ... H[i-1])` where f is some hash-like function that takes multiple inputs.
# For `i` from `1` to `M` let `R[i] = random([1 ... M])`
# `SCRYPT(D,N) = f(H[R[0]] ... H[R[M]]`. If `SCRYPT(D,N)` is low enough, then `N` is a valid nonce.
...
# Let `H[i] = sha256(D + N + i)` for `i` from 0 to 2<sup>32</sup> - 1 given data `D` and nonce `N`.
# If `abs(H[i] - H[j])` is sufficiently low, then `(N,i,j)` is a valid solution triple.

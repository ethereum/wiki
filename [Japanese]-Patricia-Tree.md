...
Merkle Patricia tree は、あらゆる (key,value) 結合の保存に使用できる、
暗号学的認証がされたデータ構造を与えます。
しかし、この仕様書においては、key 及び value は string 文字列 に限定します。
(この制限を撤廃するには、単に他のデータ型におけるシリアル化の形式を用いるだけです。)
この Merkle Patricia 木 は決定的です。というのは、全く同じ (key,value) の帳簿を持った Patricia tree は最後のバイトに至るまで正確に同じである事が保証されており、それ故同じ root hash を持ち、
また、O(log(n)) という最も速い計算時間内で挿入、検索、削除が可能であり、
red black tree のような、複雑な比較基盤のデータ構造よりも簡単に理解しコード化する事ができます。
...
これら双方の問題を解決するために、
最終的なバイトストリームにおける最初の 「ニブル（4bit)」の中に二つのフラグを符号化します。
...
符号化した後の16進文字列の長さが偶数とするために先頭から2番目のニブルを
（値０として）導入しなければなりません。
...
このアイデアは以下のとおりです。
一つずつしか要素をもたない（分岐枝を持たない）複数のノードの連なりの代わりに
 kv node `[ key, value ]` をひとつにまとめて配置します。
...
v0 から v15 までのアイテムは常にノードか空白で、
vt は常に value か空白です。
そのため、v0...v15のうちひとつのアイテムにvalueをstoreする場合、
終端記号フラグのたった空列のコンパクト符号化の結果をkeyとするkvノードを代わりに保存します。

ここに Merkle Patricia tree における node を取得するための拡張プログラムを示します:
...
ここで、node は node の内部において隠蔽されて参照されます。

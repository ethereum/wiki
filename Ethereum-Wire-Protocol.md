...
**BlockHeaders**
[`+0x0a`, [`blockHeader`, `transactionList`, `uncleList`], `...`] Specify (a) block header(s) as an answer to `GetBlocks`. The items in the list (following the message ID) are block headers in the format described in the main Ethereum specification. This may validly contain no block headers if no block headers were able to be returned for the `GetBlockHeaders` query.

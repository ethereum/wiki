...
To mine, start off by setting `nonce` at a random value and then continue incrementing the nonce until `verify(header,nonce,difficulty)` returns `1`. `verify` is defined as follows:

1. Let `h[i] = sha3(sha3(header) ++ nonce ++ i)` for `0 <= i <= 15`
2. Let `ind[i] = h[i]` modulo the number of transctions in block `N-1-i` where `N` is the current block number.
...
5. Let `v = sha3(S'.root)`
6. If `v < 2**256 / difficulty`, return `1`. Else, return `0`.

...
비트코인의 중요한 확장 기능은 블록이 여러 계층 구조(multi-layer data structure)로 저장된다는 것이다. 어떤 블록의 "해시(hash)"란 사실 블록 헤더의 해시만을 의미한다. 이 블록 헤더에는 타임스탬프, nonce, 이전 블록 해시, 그리고 블록에 포함된 모든 트랜잭션 정보에 의해 생성되는 Merkle tree의 루트 해시가 들어있다. Merkle tree는 이진트리의 일종으로서 수많은 잎사귀 노드들을 가지는 상위 노드들의 집합으로 이루어져 있으며 트리의 최하위에는 기저를 이루는 데이터(트랜잭션)가 담겨있다. 여기서 각 중간 노드들은 자기 자신 바로 하위에 있는 두 자식 노드들의 해시값이다. 최하위 기저 데이터로부터, 각 계층의 노드를 둘 씩 묶어서 해시값을 부모 노드로 하는 규칙을 반복해서 적용하면 트리의 "최상위(top)"에 있는 단 하나의 루트 노드가 남는다. Merkle tree의 목적은 어떤 블록의 데이터가 분리돼서 전달될 수 있도록 하는 것이다. 만약에 비트코인의 어떤 노드가 한 소스로부터 블록헤더만을 다운로드 받고, 이 블록헤더와 관계된 트랜잭션 정보는 다른 소스로부터 다운받아도 이 데이터들이 여전히 정확하다는 것이 보장된다. 이것이 가능한 이유는 Merkle tree에서 하위 노드들의 해시값이 상위 노드에 영향을 주기 때문에 어떤 악의적인 유저가 Merkle tree 최하위에 있는 트랜잭션 정보를 가짜로 바꿔치기 하면 상위 부모들의 해시값들이 변해서 결국 트리의 루트값이 바뀌므로, 결과적으로 이 블록의 해시가 달라지기 때문이다. 이렇게 되면 이 블록을 완전히 다른 블록으로 (51% attack이 아니라면, 유효하지 않은 작업증명으로) 등록하게 된다.

Merkle tree 프로토콜은 비트코인 네트워크를 장기간 지속가능하게 만드는 기초가 된다. 비트코인 네트워크에서 각 블록의 모든 정보를 저장하고 처리하는 "풀 노드(full node)"는 2014년 4월 기준으로 거의 15 GB의 디스크 공간을 필요로 하며 매달 1 GB 넘게 증가한다. 현재는 몇몇 데스크탑 컴퓨터에서 수용할 수 있고 스마트폰에서는 불가능하다. 그리고 나중에는 풀 노드를 유지하기 위해서, 아주 노력하는 몇몇이나 사업체들만 참여할 수 있을 것이다. "단순화된 지불증명(simplified payment verification, SPV)"으로 알려진 프로토콜은 "가벼운 노드(light node)"라고 불리는 또다른 노드 종류가 존재할 수 있게 해준다. 가벼운 노드는 블록헤더를 다운로드하고 그 블록헤더에서 proof of work를 검증한다. 그리고 관련 트랜잭션들에 대한 "곁가지들(branches)"만을 다운로드 한다. 이렇게 전체 블록체인의 매우 작은 비율만을 다운로드 함에도 불구하고 강한 안전성을 보장하면서도, 가벼운 노드가 임의의 트랜잭션의 상태 및 잔고 상태를 알아낼 수 있게 한다.

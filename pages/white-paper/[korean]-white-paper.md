...
* **블록체인 데이터를 볼 수 없다(Blockchain-blindness)**: UTXO는 논스(Nonce), 타임스탬프, 이전 블록해시 같은 블록체인 데이터를 보지 못한다. 스크립트 언어에서 무작위성 소스로 사용할 수 있는 값들이 빠지므로 도박이나 여러 종류의 앱에서 심각한 제약점이 된다.
